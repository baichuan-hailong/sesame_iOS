//
//  ZMMarketingPublishViewController.m
//  SesameService
//
...
#import "ZMMarketingPublishViewController.h"
#import "ZMPublishSuccessViewController.h"
#import "ZMMarketingView.h"

@interface ZMMarketingPublishViewController ()  <UITextFieldDelegate,
                                                 UITextViewDelegate,
                                                 UIScrollViewDelegate,
                                                 UIPickerViewDelegate,
                                                 UIPickerViewDataSource>

@property (nonatomic, strong) ZMMarketingView  *currentView;
...
@implementation ZMMarketingPublishViewController {
...
- (void)loadView {
    ...
    self.currentView = [[ZMMarketingView alloc] initWithFrame:SCREEN_BOUNDS];
    self.view        = self.currentView;
    
    self.currentView.projectNameText.delegate  = self;
    self.currentView.companyText.delegate      = self;
    self.currentView.productText.delegate      = self;
...
- (void)loadDataSources {
    ...
    NSDictionary *param = @{@"param":@"recommend"};
    [AFNetManager requestWithType:HttpRequestTypeGet withUrlString:urlStr withParaments:param withSuccessBlock:^(NSDictionary *object) {
        NSLog(@"%@",object);
        //_project = object[@"data"][@"project_type"];
...
- (void)nextStep {
    ...
    for (int i = 1; i <= 6; i++) {
        ...
        if (i == 6) {
            if ([self.currentView.priceText.text isEqualToString:@""] &&
                ...
                [toolClass showProgress:@"请填写期望推介费" toView:self.view];
                return;
            }
        } else {
            ...
            if ([currentText.text isEqualToString:@""]) {
                ...
                switch (i) {
                    case 1:
                        ...
                        break;
                    case 2:
                        [toolClass showProgress:@"请填写您要推介的公司" toView:self.view];
                        break;
                    case 3:
                        [toolClass showProgress:@"请填写您要推介的产品名称" toView:self.view];
                        break;
                    case 4:
    ...
    if (self.currentView.projectDescribeTextView.text.length < 20) {
        ...
        [toolClass showProgress:@"期望推介费金额不可大于预估标的额" toView:self.view];
        ...
        NSDictionary *param  = @{@"category":@"3",
                                 @"title":self.currentView.projectNameText.text,
                                 @"demander_name":self.currentView.companyText.text,
                                 @"type_title":self.currentView.productText.text,
...
- (void)choiceAction:(UIButton *)sender {
    ...
    if (sender.tag == 1001) {
        ...
        [UIView animateWithDuration:0.18 animations:^{
            ...
            self.currentView.priceLable.text = @"请填写推介费金额（万元）";
        ...
        [UIView animateWithDuration:0.18 animations:^{
            self.currentView.priceLable.alpha = 1;
            self.currentView.priceText.alpha  = 1;
            self.currentView.priceLable.text = @"请填写推介费比例（%）";
