...
@interface ZMCommissionDetailViewController ()<UITableViewDelegate,UITableViewDataSource>
@property (nonatomic , strong) ZMCommissionDetailView *commissionDetailView;
...
- (void)loadView {
    ...
    self.commissionDetailView    = [[ZMCommissionDetailView alloc] initWithFrame:SCREEN_BOUNDS];
    self.view                    = self.commissionDetailView;
    self.commissionDetailView.commissionDetailTableView.delegate   = self;
    self.commissionDetailView.commissionDetailTableView.dataSource = self;
}
...
- (void)finishSignNotifiAc{
    ...
    /*
     ...
     [self.commissionDetailView.commissionDetailTableView reloadData];
...
- (void)evokeSouceData{
    ...
    NSDictionary *parm = @{@"id":self.commissionID};
    NSString *urlStr   = [NSString stringWithFormat:@"%@/info/my-commit-commission-detail",APIDev];
    [AFNetManager requestWithType:HttpRequestTypeGet withUrlString:urlStr withParaments:parm withSuccessBlock:^(NSDictionary *object) {
        NSLog(@"commission-detail --- %@",object);
        ...
        if ([stateStr isEqualToString:@"success"]) {
            ...
            if (type.length>0&&![type isEqualToString:@"(null)"]) {
            ...
            //[is_show_sign integerValue]==1&&[is_show_perform integerValue]==1
...
- (void)evokeAcceptSouceData{
    ...
    NSDictionary *parm = @{@"id":self.commissionID};
    NSString *urlStr   = [NSString stringWithFormat:@"%@/info/my-accept-commission-detail",APIDev];
    [AFNetManager requestWithType:HttpRequestTypeGet withUrlString:urlStr withParaments:parm withSuccessBlock:^(NSDictionary *object) {
        NSLog(@"accept-detail --- %@",object);
        ...
        if ([stateStr isEqualToString:@"success"]) {
            ...
            if ([is_show_sign integerValue]==1&&[is_show_perform integerValue]==1) {
                self.moreArray =      @[@{@"left":@"订单进度详情",@"right":@"--"},
                                        @{@"left":@"签署居间协议",@"right":is_finish_sign},
                                        @{@"left":@"反馈项目双方签约结果",@"right":is_finish_perform}];
            }else if ([is_show_sign integerValue]==1){
                self.moreArray =      @[@{@"left":@"订单进度详情",@"right":@"--"},
                                        @{@"left":@"签署居间协议",@"right":is_finish_sign}];
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section==0) {
        ...
        if (indexPath.row==3) {
            NSDictionary *baseDic = self.twoArray[3];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section==0) {
        ZMCommissionDetailTopTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:oneIndentifire];
        if (!cell) {
            cell = [[ZMCommissionDetailTopTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:oneIndentifire];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section==2) {
        if (indexPath.row==0) {
            ...
            orderProgressVC.infoID = self.commissionID;
            orderProgressVC.isleft = _isCom;
            orderProgressVC.isYongjin = YES;
            ...
            commissionSignVC.infoID = self.commissionID;
            if ([is_finish_sign integerValue]==1) {
            ...
            feedBackSignVC.infoID = self.commissionID;
