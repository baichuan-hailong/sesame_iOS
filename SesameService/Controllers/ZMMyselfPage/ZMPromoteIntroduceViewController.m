//
//  ZMPromoteIntroduceViewController.m
//  SesameService
//
...
#import "ZMCommissionCompanyTableViewCell.h"
#import "ZMCommissionOnGoingCompanyTableViewCell.h"

#import "ZMPromoteIntroduceTableViewCell.h"
#import "ZMPromoteIntroduceOnGoingTableViewCell.h"

#import "ZMPromoteIntroduceDetailViewController.h"

@interface ZMPromoteIntroduceViewController ()<UIScrollViewDelegate,UITableViewDelegate,UITableViewDataSource>
{
    BOOL isLeftBool;
...
@implementation ZMPromoteIntroduceViewController
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (tableView.tag==626) {
        ...
        NSString *note = [NSString stringWithFormat:@"%@",leftDic[@"status_note"][@"note"]];
        if (note.length>0&&![note isEqualToString:@"(null)"]) {
            ZMCommissionOnGoingCompanyTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:leftOngoIndentifire];
            if (!cell) {
                cell = [[ZMCommissionOnGoingCompanyTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:leftOngoIndentifire];
                cell.selectionStyle = UITableViewCellSelectionStyleNone;
            }
            
            [cell setCommission:leftDic isLeft:YES];
            return cell;
        }else{
            ZMCommissionCompanyTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:leftIndentifire];
            if (!cell) {
                cell = [[ZMCommissionCompanyTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:leftIndentifire];
                cell.selectionStyle = UITableViewCellSelectionStyleNone;
            }
        ...
        if (note.length>0&&![note isEqualToString:@"(null)"]) {
            ...
            [cell setCommission:rightDic isLeft:NO isCom:NO];
            return cell;
        }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableView.tag==626) {
        NSLog(@"left");
        NSDictionary *leftDic = self.leftMutableArray[indexPath.section];
        ZMPromoteIntroduceDetailViewController *promoteIntroduceDetailVC = [[ZMPromoteIntroduceDetailViewController alloc] init];
        promoteIntroduceDetailVC.promoteID = [NSString stringWithFormat:@"%@",leftDic[@"id"]];
        promoteIntroduceDetailVC.isCom        = YES;
        [self.navigationController pushViewController:promoteIntroduceDetailVC animated:YES];
    }else{
        NSLog(@"right");
        NSDictionary *rightDic = self.rightMutableArray[indexPath.section];
        ZMPromoteIntroduceDetailViewController *promoteIntroduceDetailVC = [[ZMPromoteIntroduceDetailViewController alloc] init];
        promoteIntroduceDetailVC.promoteID = [NSString stringWithFormat:@"%@",rightDic[@"id"]];
        promoteIntroduceDetailVC.isCom     = NO;
        promoteIntroduceDetailVC.isGoing   =YES;
        [self.navigationController pushViewController:promoteIntroduceDetailVC animated:YES];
    }
}
...
- (void)evokeLeftSouceData{
    ...
    NSString *urlStr   = [NSString stringWithFormat:@"%@/info/my-commit-recommend-lists",APIDev];
...
- (void)evokeRightSouceData{
    ...
    NSDictionary *parm = @{@"page":@"1"};
    NSString *urlStr   = [NSString stringWithFormat:@"%@/info/my-accept-recommend-lists",APIDev];
    [AFNetManager requestWithType:HttpRequestTypeGet withUrlString:urlStr withParaments:parm withSuccessBlock:^(NSDictionary *object) {
        NSLog(@"right --- %@",object);
        NSLog(@"%@",object[@"status"][@"message"]);
