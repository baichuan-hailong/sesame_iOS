//
//  ZMCommissionOnGoingTableViewCell.m
//  SesameService
//
...
@implementation ZMCommissionOnGoingTableViewCell
    ...
    if (isLeft) {
        ...
        if (isCom) {
            ...
            if ([reward_type isEqualToString:@"fixed_value"]) {
                ...
                //按行业惯例或接单方规定
                self.accountedLabel.text = @"期望佣金：按行业惯例或接单方规定";
            }else if ([reward_type isEqualToString:@"negotiable"]) {
                //认可平台协商的结果
                self.accountedLabel.text = @"期望佣金：认可平台协商的结果";
            }
        }else{
            if ([reward_type isEqualToString:@"fixed_value"]) {
                //按金额
